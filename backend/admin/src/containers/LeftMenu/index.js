...
const LeftMenu = forwardRef(({ shouldUpdateStrapi, version, plugins }, ref) => {
  ...
  useEffect(() => {
    pluginsSectionLinksFiltered.map(({ label: { id } }, idx) => {
      if (id == "commerce.plugin.name") {
        setCommerce([pluginsSectionLinksFiltered[idx]]);
        pluginsSectionLinksFiltered.splice(idx, 1);
      }
    });
  });
  return (
